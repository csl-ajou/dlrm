...
class DLRM_Net(nn.Module):
    ...
    def apply_emb(self, lS_o, lS_i, emb_l, v_W_l):
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            ...
            if self.quantize_emb:
                ...
                # print("mem size of V", V.detach().cpu().numpy().nbytes)
                # print("mem size of V, sys", sys.getsizeof(V))
                # print("mem size of V, package", get_deep_size(V))
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i):
        ...
        print("--------apply_mlp-------")
        with record_function("FOR apply_top"):
        # d = torch.device("cuda", 0)
        # execute it on GPU
        # dense_x = dense_x.to(d)
        # self.bot_l = self.bot_l.to(d)
            x = self.apply_mlp(dense_x, self.bot_l)
        # debug prints
        #print("intermediate")
        ...
        print("-----apply emb------")
        # self.emb_l.cuda_host()
        with record_function("FOR apply_emb"):
            ly = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l) # apply_emb executes on CPU
        # print(ly)
        # ly = [_ly.cuda() for _ly in ly] # send lookup and reduction result to GPU 
        # ly = torch.stack(ly)
        # ly = ly.to(d)
        ...
        # execute it on GPU
        print("-----interact features------")
        with record_function("FOR interact features"):
            z = self.interact_features(x, ly)
        # z = z.to(d)
        ...
        print("-----apply top mlp------")
        # self.top_l = self.top_l.to(d)
        with record_function("FOR mlp top"):
            p = self.apply_mlp(z, self.top_l)
...
def run():
    ...
    def travel_layers(mod, num = [0]):
                ...
                if len(list(mod.children())) == 0:
                    ...
                    for child in mod.children():
                        layers += travel_layers(child, num)
                return layers
    ...
    if use_gpu:
        ...
        layers = travel_layers(dlrm)
        print(layers)
        # print(layers[0][1])
        # print(type(layers[0][1]))
        for i in range(len(layers)):
            if isinstance(layers[i][1], torch.nn.modules.sparse.EmbeddingBag):
                print("EmbeddingBag!")
                continue
            else:
                layers[i][1].to(device)
        # dlrm = dlrm.to(device)  # .cuda() # should be fixed
        print("dlrm.ndevices: ", dlrm.ndevices)
        if dlrm.ndevices > 1:
